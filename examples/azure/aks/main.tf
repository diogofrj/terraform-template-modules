...
resource "azurerm_kubernetes_cluster" "aks_cluster" {
  ...
  location                          = var.location
  resource_group_name               = try(var.create_resource_group == false ? data.azurerm_resource_group.rgrp[0].name : azurerm_resource_group.rg[0].name, var.resource_group_name)
