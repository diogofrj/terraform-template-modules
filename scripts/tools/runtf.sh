...
plan() {
    ...
    terraform -chdir="$TERRAFORM_DIR" plan -no-color -out=plan/tf.plan #-var-file=common.tfvars
    terraform -chdir="$TERRAFORM_DIR" show -json plan/tf.plan | jq > plan/tf_plan.json
    terraform -chdir="$TERRAFORM_DIR" show -no-color plan/tf.plan > plan/tf_plan.hcl
}
show() {
    echo -e "${YELLOW}Executando Terraform Show...${NC}"
    terraform -chdir="$TERRAFORM_DIR" show -no-color -json plan/tf.plan | jq
}

apply() {
    echo -e "${YELLOW}Executando Terraform Apply...${NC}"
    terraform -chdir="$TERRAFORM_DIR" apply -auto-approve plan/tf.plan #-var-file=common.tfvars
}
outputs() {
...
backup() {
    echo -e "${YELLOW}Iniciando backup do diretório atual...${NC}"

    # Define as variáveis
    PASTA_ORIGEM=$(pwd)
    DESTINO="${PASTA_ORIGEM}/backups"
    DATA=$(date +%Y%m%d_%H%M%S)
    NOME_ARQUIVO="terraform_backup_$DATA.tar.gz"
    ...
    # Cria o backup
    echo -e "${YELLOW}Criando backup em: $DESTINO/$NOME_ARQUIVO${NC}"
    tar czf "$DESTINO/$NOME_ARQUIVO" "${EXCLUSOES[@]}" .

    # Remove backups mais antigos que 30 dias
    find "$DESTINO" -name "terraform_backup_*.tar.gz" -mtime +30 -delete

    echo -e "${GREEN}Backup concluído com sucesso!${NC}"
    echo -e "${GREEN}Arquivo: $DESTINO/$NOME_ARQUIVO${NC}"
}
...
main() {
    if [ $# -eq 0 ]; then
        ...
        echo -e "    backup                   - Cria backup do diretório atual"
        exit 1
    fi
